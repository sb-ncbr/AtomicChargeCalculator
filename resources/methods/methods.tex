...
\chapter*{A sort description of the methods used in ACC III}

\tableofcontents*

\chapternonum{Empirical charge calculation methods}
This section shortly describes the main idea of each method implemented in Atomic Charge Calculator~III. Methods here are ordered according to the publication date.
...
\section*{SQE+qp}
SQE+qp \cite{Schindler2021} replaces the formal charges $q_0$ with the parameterized ones $q_p$. To preserve the total charge of the molecule, these parameterized values must be normalized first:
...
\chapternonum{Complexity of implemented methods}

The following table summarizes computational costs of the implemented methods, both in terms of time and memory complexity (expressed using ${\mathcal O}$ notation). The symbol $N$ denotes the number of atoms, $M$ is the number of bonds. Methods are grouped according to whether they depend on molecular geometry (3D) or not (2D).
...
\begin{tabular}{llll}
\toprule
\multicolumn{4}{l}{\textbf{2D methods (topology-based)}} \\
\midrule
Method & Time & Memory & Suitable for large systems \\
\midrule
DelRe & $N^3$ & $N^2$ & no \\
PEOE  & $N + M$ & $N$ & yes \\
Charge2 & $N + M$ & $1$ & yes \\
...
VEEM & $N$ & $1$ & yes \\
\addlinespace
\multicolumn{4}{l}{\textbf{3D methods (geometry-dependent)}} \\
\midrule
Method & Time & Memory & Suitable for large systems \\
\midrule
GDAC & $N + M$ & $N$ & yes \\
...
In addition to asymptotic complexity, practical applicability depends on the availability of complexity-reduction techniques. Methods marked with * support cutoff and cover schemes, which significantly reduce computational cost and memory usage for large systems (see the details in the next \hyperref[chap:cutoff]{section}). Methods without these techniques (e.g., SQE and related variants) exhibit unfavourable scaling and are therefore not recommended for large molecules ($N > 20 000$ atoms). This classification is reflected in ACC III, where such methods are excluded from default selection for large systems. However, when using ChargeFW2 locally, users may override this behaviour if needed.


\chapternonum{Illustrative performance benchmarks}

To complement the asymptotic complexity analysis, we provide \textit{illustrative} benchmark measurements for selected methods (PEOE, EEM, and SQE+qp). All measurements were performed on a single CPU core of an AMD Ryzen 7 9700X processor with 64 GB RAM, Fedora 43, using the CLI ChargeFW2 application. Reported runtimes correspond to wall-clock time.
...
\toprule
System & Method & Atoms & Time (s) & Memory & Unit & Note \\
\midrule
\multirow{3}{*}{Small molecule}
 & PEOE   & 17    & $<0.01$ & 7   & MB & -- \\
 & EEM    & 17    & $<0.01$ & 6   & MB & -- \\
 & SQE+qp & 17    & $<0.01$ & 7   & MB & -- \\
\addlinespace
\multirow{3}{*}{Protein}
 & PEOE   & 5761  & 0.02    & 18  & MB & -- \\
 & EEM    & 5761  & 1.4     & 530 & MB & -- \\
 & SQE+qp & 5761  & 7.1     & 1055& MB & -- \\
\addlinespace
\multirow{3}{*}{Large protein}
 & PEOE   & 40738 & 0.16    & 97  & MB & -- \\
 & EEM    & 40738 & 36.2    & 98  & MB & Cutoff used* \\
 & SQE+qp & 40738 & 391.7   & 36.7& GB & **\\
\bottomrule
\end{tabular}
\end{center}

* For supported methods (see the \hyperref[tab:complexities]{table}), cutoff is automatically used for molecules having more than $20 000$ atoms.

** This option is not considered suitable for such a molecule by default, and therefore not listed on the ACC III web site. It has to be selected manually, when using ChargeFW2 locally.

\chapternonum{Assessment of charges quality}
...
\chapternonum{Cutoff and cover approaches}
\label{chap:cutoff}

Solving the electronegativity equalization system of linear equations can be troublesome for very large structures as it requires, in general, ${\mathcal O}(N^3)$ steps and ${\mathcal O}(N^2)$ memory. The original AtomicChargeCalculator (ACC) introduced two \textit{divide and conquer} complexity reduction algorithms to overcome this issue. Since ACC only supports EEM, these approaches were called \textit{EEM Cutoff} and \textit{EEM Cover}. ACC~III extended these approaches to other applicable methods, referencing them simply as \textit{cutoff} and \textit{cover}.
...
\chapternonum{Notes on the implementation}
...
\chapternonum{Applications of charges}
...
\chapternonum{Overview of parameter sets}

The following table summarises the parameter sets available in Atomic Charge Calculator III. The name is a clickable link to the publication where the parameter set was introduced. The \emph{Coverage} column indicates the chemical elements to which each parameter set applies. The symbol ${}^{*}$ denotes parameter sets with additional restrictions on atom environments (typically related to the highest bond order), while ${}^{+}$ indicates parameter sets with additional restrictions on bond types. For full details, refer to the individual parameter files available on \href{https://github.com/sb-ncbr/ChargeFW2/tree/main/data/parameters}{GitHub}.

\begin{longtable}{p{4cm} p{3.3cm} p{8cm}}
\toprule
Name & Coverage & Notes \\
\midrule
\endfirsthead
...
\bottomrule
\endlastfoot
\addlinespace[5pt]\multicolumn{3}{l}{\textbf{ABEEM}} \\\addlinespace[5pt]
\addlinespace[5pt] \href{https://doi.org/10.1021/jp9711048}{Yang 1997} & C, H, N, O$^{*,+}$ & Parameterised to reproduce HF/STO-3G/MPA charges \\
\addlinespace
\hline\hline
\addlinespace[5pt]\multicolumn{3}{l}{\textbf{Charge2}} \\\addlinespace[5pt]
\addlinespace[5pt] \href{https://doi.org/10.1002/jcc.540030316}{Abraham 1982} & Br, C, Cl, F, H, I, N, O & Mulliken-derived electronegativities; empirical polarizabilities \\
\addlinespace
\hline\hline
\addlinespace[5pt]\multicolumn{3}{l}{\textbf{Del Re}} \\\addlinespace[5pt]
\addlinespace[5pt] \href{https://doi.org/10.1039/JR9580004031}{Del Re 1958} & C, Cl, F, H, N, O$^{+}$ & Electronegativity-based, heuristically tuned to dipole moments \\
\addlinespace
\hline\hline
\addlinespace[5pt]\multicolumn{3}{l}{\textbf{DENR}} \\\addlinespace[5pt]
\addlinespace[5pt] \href{https://doi.org/10.1021/j100161a070}{Rappe 1991} & Br, C, Cl, Cs, F, H, I, K, Li, N, Na, O, P, Rb, S, Si & Derived from experimental atomic IPs and EAs \\
\addlinespace
\hline\hline
\addlinespace[5pt]\multicolumn{3}{l}{\textbf{EEM}} \\\addlinespace[5pt]
\addlinespace[5pt] \href{https://doi.org/10.1021/ja00018a003}{Baekelandt 1991} & Al, C, H, N, O, P, Si$^{*}$ & Parameterised to reproduce HF/STO-3G/MPA charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1021/jp020547v}{Bultinck 2002}\\(CHELPG) & C, F, H, N, O & Parameterised to reproduce B3LYP/6-31G*/CHELPG charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1021/jp020547v}{Bultinck 2002}\\(Hirshfeld) & C, F, H, N, O & Parameterised to reproduce B3LYP/6-31G*/Hirshfeld charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1021/jp020547v}{Bultinck 2002}\\(MK) & C, F, H, N, O & Parameterised to reproduce B3LYP/6-31G*/MK charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1021/jp020547v}{Bultinck 2002}\\(Mulliken) & C, F, H, N, O & Parameterised to reproduce B3LYP/6-31G*/MPA charges \\
\addlinespace[5pt] \href{https://doi.org/10.1021/jp020547v}{Bultinck 2002}\\(NPA) & C, F, H, N, O & Parameterised to reproduce B3LYP/6-31G*/NPA charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1021/jp046928l}{Bultinck 2004}\\(AIM) & C, F, H, N, O & Parameterised to reproduce B3LYP/6-31G*/AIM charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1186/s13321-015-0107-1}{Geidl 2015}\\(Cheminf\_b3lyp\_aim) & Br, C, Cl, F, H, I, N, O, P, S$^{*}$ & Parameterised to reproduce B3LYP/6-311G/AIM charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1186/s13321-015-0107-1}{Geidl 2015}\\(Cheminf\_b3lyp\_mpa) & Br, C, Cl, F, H, I, N, O, P, S$^{*}$ & Parameterised to reproduce B3LYP/6-311G/MPA charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1186/s13321-015-0107-1}{Geidl 2015}\\(Cheminf\_b3lyp\_npa) & Br, C, Cl, F, H, I, N, O, P, S$^{*}$ & Parameterised to reproduce B3LYP/6-311G/NPA charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1186/s13321-015-0107-1}{Geidl 2015}\\(Cheminf\_hf\_aim) & Br, C, Cl, F, H, I, N, O, P, S$^{*}$ & Parameterised to reproduce HF/6-311G/AIM charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1186/s13321-015-0107-1}{Geidl 2015}\\(Cheminf\_hf\_mpa) & Br, C, Cl, F, H, I, N, O, P, S$^{*}$ & Parameterised to reproduce HF/6-311G/MPA charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1186/s13321-015-0107-1}{Geidl 2015}\\(Cheminf\_hf\_npa) & Br, C, Cl, F, H, I, N, O, P, S$^{*}$ & Parameterised to reproduce HF/6-311G/NPA charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1021/ci400448n}{Ionescu 2013}\\(EX-MPA\_6-31Gd\_PCM) & C, Ca, H, N, O, S$^{*}$ & Parameterised to reproduce HF/6-31G**/MPA/polarisable-continuum-model charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1021/ci400448n}{Ionescu 2013}\\(EX-MPA\_6-31Gd\_gas) & C, Ca, H, N, O, S$^{*}$ & Parameterised to reproduce HF/6-31G**/MPA/gas-phase charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1021/ci400448n}{Ionescu 2013}\\(EX-NPA\_6-31Gd\_PCM) & C, Ca, H, N, O, S$^{*}$ & Parameterised to reproduce HF/6-31G**/NPA/polarisable-continuum-model charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1021/ci400448n}{Ionescu 2013}\\(EX-NPA\_6-31Gd\_gas) & C, Ca, H, N, O, S$^{*}$ & Parameterised to reproduce HF/6-31G**/NPA/gas-phase charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1039/b821696g}{Ouyang 2009}\\(124 calibrated set) & C, H, N, O & Parameterised to reproduce B3LYP/6-31G*/NPA charges \\
\pagebreak
\href{https://doi.org/10.1039/b821696g}{Ouyang 2009}\\(131 calibrated set) & C, F, H, N, O & Parameterised to reproduce B3LYP/6-31G*/NPA charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1039/b821696g}{Ouyang 2009}\\(hybridization-dependent) & C, H, N, O$^{*}$ & Parameterised to reproduce B3LYP/6-31G*/NPA charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1186/s13321-016-0171-1}{Racek 2016}\\(ccd2016\_mpa) & Br, C, Cl, F, H, N, O, P, S$^{*}$ & Parameterised to reproduce B3LYP/6-311G/MPA charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1186/s13321-016-0171-1}{Racek 2016}\\(ccd2016\_mpa2) & Br, C, Cl, F, H, N, O, P, S$^{*}$ & Parameterised to reproduce B3LYP/6-311G*/MPA charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1186/s13321-016-0171-1}{Racek 2016}\\(ccd2016\_npa) & Br, C, Cl, F, H, N, O, P, S$^{*}$ & Parameterised to reproduce B3LYP/6-311G/NPA charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1186/s13321-016-0171-1}{Racek 2016}\\(ccd2016\_npa2) & Br, C, Cl, F, H, N, O, P, S$^{*}$ & Parameterised to reproduce B3LYP/6-311G*/NPA charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.3390/i8070572}{Svobodova 2007}\\(cbeg2) & C, H, N, O, S$^{*}$ & Parameterised to reproduce HF/STO-3G/MPA charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.3390/i8070572}{Svobodova 2007}\\(chal2) & Br, C, Cl, F, H, I, N, O, S$^{*}$ & Parameterised to reproduce HF/STO-3G/MPA charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.3390/i8070572}{Svobodova 2007}\\(chm2) & Br, C, Cl, F, Fe, H, I, N, O, S, Zn$^{*}$ & Parameterised to reproduce HF/STO-3G/MPA charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.3390/i8070572}{Svobodova 2007}\\(cmet2) & C, Fe, H, N, O, S, Zn$^{*}$ & Parameterised to reproduce HF/STO-3G/MPA charges \\
\addlinespace
\hline\hline
\addlinespace[5pt]\multicolumn{3}{l}{\textbf{EQeq+C}} \\\addlinespace[5pt]
\addlinespace[5pt] \href{https://doi.org/10.1021/acs.jctc.5b00037}{Martin-Noble 2015}\\(ATMO/H-I) & C, H, N, O, Se, Te, V & Bond-order corrections for EQEq, parameterised to reproduce ATMO/Hirshfeld-I charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1021/acs.jctc.5b00037}{Martin-Noble 2015}\\(MOF/REPEAT) & C, Co, Cu, H, Mg, N, Ni, O, Pd, V, Zn & Bond-order corrections for EQEq, parameterised to reproduce MOF/REPEAT charges \\
\addlinespace
\hline\hline
\addlinespace[5pt]\multicolumn{3}{l}{\textbf{GDAC}} \\\addlinespace[5pt]
\addlinespace[5pt] \href{https://doi.org/10.1021/jp0023213}{Cho 2001} & C, H, N, O, S$^{*}$ & Parameterised to reproduce B3LYP/6-31G** dipole moments and MPA charges \\
\addlinespace[5pt]\multicolumn{3}{l}{\textbf{KCM}} \\\addlinespace[5pt]
\addlinespace[5pt] \href{https://doi.org/10.1007/BFb0029840}{Yakovenko 2008}\\(initial) & C, H, N, O, S$^{*}$ & Parameterised to reproduce MMFF94 charges \\
\addlinespace
\hline\hline
\addlinespace[5pt]\multicolumn{3}{l}{\textbf{MPEOE}} \\\addlinespace[5pt]
\addlinespace[5pt] \href{https://doi.org/10.1021/j100374a066}{No 1990}\\(DP1) & C, H, N, O, S$^{*,+}$ & Empirically fitted to dipole (and quadrupole) moments \\
\addlinespace
\hline\hline
\addlinespace[5pt]\multicolumn{3}{l}{\textbf{PEOE}} \\\addlinespace[5pt]
\addlinespace[5pt] \href{https://doi.org/10.1016/0040-4020(80)80168-2}{Gasteiger 1980} & Br, C, Cl, F, H, I, N, O, S$^{*}$ & Derived directly from IPs and EAs \\
\addlinespace
\hline\hline
\addlinespace[5pt]\multicolumn{3}{l}{\textbf{QEq}} \\\addlinespace[5pt]
\addlinespace[5pt] \href{https://doi.org/10.1021/j100161a070}{Rappe 1991} & Br, C, Cl, Cs, F, H, I, K, Li, N, Na, O, P, Rb, S, Si & Derived from experimental atomic IPs and EAs \\
\addlinespace
\hline\hline
\addlinespace[5pt]\multicolumn{3}{l}{\textbf{SFKEEM}} \\\addlinespace[5pt]
\addlinespace[5pt] \href{https://doi.org/10.1021/ci050505e}{Chaves 2006} & C, F, H, N, O & Parameterised to reproduce B3LYP/6-31G*/MPA charges \\
\addlinespace
\hline\hline
\addlinespace[5pt]\multicolumn{3}{l}{\textbf{SMP/QEq}} \\\addlinespace[5pt]
\addlinespace[5pt] \href{https://doi.org/10.1021/jp8063273}{Zhang 2009} & Ag, Al, Au, Cu, K, Li, Na, Rb & Fitted to DFT (SVWN) energy derivatives \\
\addlinespace
\hline\hline
\addlinespace[5pt]\multicolumn{3}{l}{\textbf{SQE}} \\\addlinespace[5pt]
\addlinespace[5pt] \href{https://doi.org/10.1186/s13321-021-00528-w}{Schindler 2021}\\(CCD\_gen) & Br, C, Cl, F, H, N, O, P, S$^{*,+}$ & Parameterised to reproduce B3LYP/6-311G/NPA charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1186/s13321-021-00528-w}{Schindler 2021}\\(DTP\_small) & C, H, N, O, S$^{*,+}$ & Parameterised to reproduce B3LYP/6-311G/NPA charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1186/s13321-021-00528-w}{Schindler 2021}\\(PUB\_pept) & C, H, N, O, S$^{*,+}$ & Parameterised to reproduce B3LYP/6-31G*/NPA charges \\
\addlinespace
\hline\hline
\addlinespace[5pt]\multicolumn{3}{l}{\textbf{SQE+q0}} \\\addlinespace[5pt]
\addlinespace[5pt] \href{https://doi.org/10.1186/s13321-021-00528-w}{Schindler 2021}\\(CCD\_gen) & Br, C, Cl, F, H, N, O, P, S$^{*,+}$ & Parameterised to reproduce B3LYP/6-311G/NPA charges \\
\pagebreak
\addlinespace[5pt] \href{https://doi.org/10.1186/s13321-021-00528-w}{Schindler 2021}\\(DTP\_small) & C, H, N, O, S$^{*,+}$ & Parameterised to reproduce B3LYP/6-311G/NPA charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1186/s13321-021-00528-w}{Schindler 2021}\\(PUB\_pept) & C, H, N, O, S$^{*,+}$ & Parameterised to reproduce B3LYP/6-31G*/NPA charges \\
\addlinespace
\hline\hline
\addlinespace[5pt]\multicolumn{3}{l}{\textbf{SQE+qp}} \\\addlinespace[5pt]
\addlinespace[5pt] \href{https://doi.org/10.1186/s13321-021-00528-w}{Schindler 2021}\\(CCD\_gen) & Br, C, Cl, F, H, N, O, P, S$^{*,+}$ & Parameterised to reproduce B3LYP/6-311G/NPA charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1186/s13321-021-00528-w}{Schindler 2021}\\(DTP\_small) & C, H, N, O, S$^{*,+}$ & Parameterised to reproduce B3LYP/6-311G/NPA charges \\
\addlinespace[5pt]\midrule\addlinespace[5pt] \href{https://doi.org/10.1186/s13321-021-00528-w}{Schindler 2021}\\(PUB\_pept) & C, H, N, O, S$^{*,+}$ & Parameterised to reproduce B3LYP/6-31G*/NPA charges \\
\addlinespace
\hline\hline
\addlinespace[5pt]\multicolumn{3}{l}{\textbf{TSEF}} \\\addlinespace[5pt]
\addlinespace[5pt] \href{https://doi.org/10.1021/j100161a070}{Rappe 1991} & Br, C, Cl, Cs, F, H, I, K, Li, N, Na, O, P, Rb, S, Si & Derived from experimental atomic IPs and EAs \\
\addlinespace
\end{longtable}
